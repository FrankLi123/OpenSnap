...
async def page_snap(page: Page) -> str:
    ...
    for img in await page.query_selector_all('img[src]'):
        ...
        if full_src:
            ...
            content = content.replace(src,
                                      embed_resource(resources[full_src]['content_type'], resources[full_src]['data']))
        else:
            print('资源未命中，单独发起请求')
            try:
                # response = await page.evaluate('(src) => fetch(src).then(r => r.arrayBuffer()).then(r => btoa(String.fromCharCode(...new Uint8Array(r))))', src)
                response = await page.evaluate_handle(
                    '(src) => fetch(src).then(r => r.arrayBuffer()).then(r => btoa(String.fromCharCode(...new Uint8Array(r))))',
                    src)
    ...
    def link_script_filter(tag):
        return tag.name == "link" and (tag.has_attr("as") and tag["as"] == "script") or (
                    tag.has_attr('href') and tag['href'].endswith('.js'))
    ...
    def ssr_dns_prefetch_filter(tag):
        return tag.name == "link" and (tag.has_attr("data-n-head") and tag["data-n-head"] == "ssr") or (
                    tag.has_attr("rel") and "dns-prefetch" in tag["rel"])
...
def main():
    ...
    asyncio.run(save_as_single_file(
        'https://github.com/langchain-ai/langchain','test.html'
    ))
